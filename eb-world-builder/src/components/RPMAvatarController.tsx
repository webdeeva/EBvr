...
interface RPMAvatarControllerProps {
  avatarUrl: string;
  position: [number, number, number];
}

const RPMAvatarController: React.FC<RPMAvatarControllerProps> = ({ avatarUrl, position }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (direction.current.length() > 0) {
      ...
      // Simple walking effect - slight bobbing
      const time = state.clock.getElapsedTime();
      const bobAmount = 0.03;
      const bobSpeed = isMoving && keys.current.shift ? 15 : 10;
      avatarRef.current.position.y = position[1] + Math.abs(Math.sin(time * bobSpeed)) * bobAmount;
    } else {
...
export default RPMAvatarController;